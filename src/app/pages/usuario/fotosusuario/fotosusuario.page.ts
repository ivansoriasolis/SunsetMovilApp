...
@Component({
  selector: 'app-fotosusuario',
  templateUrl: './fotosusuario.page.html',
  styleUrls: ['./fotosusuario.page.scss'],
})
export class FotosusuarioPage implements OnInit {
  uidUsuario = '';
  publicaciones: any[] = [];
  comentarios: any[] = [];
  ...
  async cargarModulo(evento: any = CustomEvent) {
    this.moduloCargado = false;
    this.uidUsuario = this.publicacionService.uidUsuarioVerPublicaciones;
    this.publicaciones =
      await this.publicacionService.obtenerPublicacionesPersonales(
        this.uidUsuario
      );
    ...
    const alert = await this.alertController.create({
      ...
      buttons: [
        ...
        },
        {
          ...
          role: 'confirm',
          handler: async (datos) => {
            await this.reportarPublicacion(datos.reporte);
  ...
  async reportarPublicacion(razonReporte: string) {
    let datosReporte = {
    ...
    await this.publicacionService.reportarPublicacion(datosReporte);
  }
